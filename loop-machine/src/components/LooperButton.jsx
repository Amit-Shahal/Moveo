...
export default function LooperButton(props) {
  ...
  const togglePlayPause = () => {
    ...
    if (prevValue === isPlayingOption[0]) {
      if (maxDuration.duration === -1) {
        ...
        setIsPlaying(isPlayingOption[2]); //queue
      }
    }
    //when on or queue
  ...
  const audioEnded = () => {
    // if (audioPlayer.current.src === maxDuration.src) {
    //   //handle bug fix - when alot of audio are queued thay cuase a delay with onEnded Func. so if its max duration then start now
    //   setIsPlaying(isPlayingOption[1]); //on
    // } else {
    setIsPlaying(isPlayingOption[2]); //queue
    // }
